...
void WindowsTaskbarPlugin::HandleMethodCall(
  ...
  if (windows_taskbar_ == nullptr) {
    ...
    try {
      ...
      if (view == nullptr) {
        result->Error("-1", "ERROR: Window view not initialized yet");
        return;
      }
      auto native_window = view->GetNativeWindow();
      if (native_window == nullptr) {
        result->Error("-1", "ERROR: Native window not available yet");
        return;
      }
      hwnd = ::GetAncestor(native_window, GA_ROOT);
      if (hwnd == nullptr || !::IsWindow(hwnd)) {
        result->Error("-1", "ERROR: Invalid window handle");
        return;
      }
    } catch (...) {
      result->Error("-1", "ERROR: Failed to get window handle");
      return;
    }
    
    try {
      windows_taskbar_ = std::make_unique<WindowsTaskbar>(hwnd);
    } catch (...) {
      result->Error("-1", "ERROR: Failed to initialize WindowsTaskbar");
      return;
    }
  }
  auto arguments = std::get<flutter::EncodableMap>(*method_call.arguments());
  
  // 包装所有操作在try-catch中,确保异常能被正确捕获
  try {
    ...
    result->Error("-1", std::string("ERROR: WindowsTaskbar exception: ") + e.what());
  } catch (...) {
    result->Error("-1", "ERROR: WindowsTaskbar unknown exception");
  }
}

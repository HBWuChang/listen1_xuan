...
const double _kSweep = _kTwoPi - _kEpsilon;
const double _kCircularStartAngle = -math.pi / 2.0;

enum _ActivityIndicatorType { material, adaptive }
enum _IndicatorMode { determinate, indeterminate, transitioningToDeterminate, transitioningToIndeterminate }
...
class LinearProgressIndicator extends ProgressIndicator {
  ...
  /// The duration of the transition when switching between indeterminate and
  /// determinate states.
  final Duration? transitionDuration;

  /// The curve used for the transition when switching states.
...
class _LinearProgressIndicatorState extends State<LinearProgressIndicator>
  ...
  _IndicatorMode _mode = _IndicatorMode.indeterminate;
  Animation<double>? _valueAnimation;
  double? _lastDeterminateValue;
  ...
  void initState() {
    ...
    _mode = widget.value == null ? _IndicatorMode.indeterminate : _IndicatorMode.determinate;
    if (widget.value == null) {
      _controller.repeat();
    } else {
      _lastDeterminateValue = widget.value;
    }
  }
  ...
  void didUpdateWidget(LinearProgressIndicator oldWidget) {
    ...
    _transitionController.duration = _effectiveTransitionDuration;
    final double? oldValue = oldWidget.value;
    final double? newValue = widget.value;

    if (newValue != null) {
      _lastDeterminateValue = newValue;
    }

    if (oldValue == null && newValue == null) {
      if (!_controller.isAnimating) {
        _mode = _IndicatorMode.indeterminate;
        _controller.repeat();
      }
      return;
    }
    ...
    if (oldValue == null && newValue != null) {
      _startTransitionToDeterminate(newValue);
      return;
    }

    if (oldValue != null && newValue == null) {
      _startTransitionToIndeterminate(oldValue);
      return;
    }

    if (oldValue != null && newValue != null && oldValue != newValue) {
      _startTransitionBetweenDeterminate(oldValue, newValue);
      return;
    }
  }
  ...
  Curve get _effectiveTransitionCurve => widget.transitionCurve ?? Curves.easeInOut;

  double _clampValue(double value) => clampDouble(value, 0.0, 1.0);

  double _currentLinearValueForTransition(double fallbackValue) {
    if ((_mode == _IndicatorMode.transitioningToDeterminate ||
            _mode == _IndicatorMode.transitioningToIndeterminate) &&
        _transitionController.isAnimating &&
        _valueAnimation != null) {
      return _valueAnimation!.value;
    }

    if (_mode == _IndicatorMode.determinate) {
      return _clampValue(fallbackValue);
    }

    return _clampValue(fallbackValue);
  }

  void _resetTransitionAnimation(Tween<double> tween) {
    _transitionController.stop();
    _valueAnimation = tween.chain(CurveTween(curve: _effectiveTransitionCurve)).animate(_transitionController)
      ..addListener(() => setState(() {}));
  }

  void _startTransitionToDeterminate(double targetValue) {
    _controller.stop();
    final double begin = _currentLinearValueForTransition(_lastDeterminateValue ?? targetValue);
    final double end = _clampValue(targetValue);

    _resetTransitionAnimation(Tween<double>(begin: begin, end: end));

    _mode = _IndicatorMode.transitioningToDeterminate;
    _transitionController
      ..reset()
      ..forward().whenComplete(() {
        if (!mounted) return;
        setState(() {
          _mode = _IndicatorMode.determinate;
          _valueAnimation = null;
        });
      });
  }

  void _startTransitionToIndeterminate(double fromValue) {
    _controller.stop();
    final double begin = _currentLinearValueForTransition(fromValue);
  ...
  void _startTransitionBetweenDeterminate(double fromValue, double toValue) {
    _controller.stop();
    final double begin = _currentLinearValueForTransition(fromValue);
    final double end = _clampValue(toValue);
    ...
    Widget result = ConstrainedBox(
      ...
      child: CustomPaint(
        painter: _LinearProgressIndicatorPainter(
          ...
          value: valueOverride ?? widget.value, // may be null
          animationValue: animationValue, // ignored if value is not null
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.value != null) {
      return _buildIndicator(
        ...
        _controller.value,
        textDirection,
        valueOverride: _mode == _IndicatorMode.transitioningToDeterminate
            ? _valueAnimation?.value ?? widget.value
            : widget.value,
      );
    }

    if (_mode == _IndicatorMode.transitioningToIndeterminate ||
        _mode == _IndicatorMode.transitioningToDeterminate) {
      return _buildIndicator(
        context,
        0.0,
        textDirection,
        valueOverride: _valueAnimation?.value,
      );
    }
...
class _CircularProgressIndicatorPainter extends CustomPainter {
  _CircularProgressIndicatorPainter({
    ...
    this.trackGap,
    this.year2023 = true,
    this.arcStartOverride,
    this.arcSweepOverride,
  }) : arcStart = value != null
         ? (arcStartOverride ?? _kCircularStartAngle)
         : (arcStartOverride ?? (_kCircularStartAngle +
                 tailValue * 3 / 2 * math.pi +
                 rotationValue * math.pi * 2.0 +
           offsetValue * 0.5 * math.pi)),
       arcSweep = value != null
         ? (arcSweepOverride ?? clampDouble(value, 0.0, 1.0) * _kSweep)
         : (arcSweepOverride ?? math.max(headValue * 3 / 2 * math.pi - tailValue * 3 / 2 * math.pi, _kEpsilon));
  ...
  final double? trackGap;
  final bool year2023;
  final double? arcStartOverride;
  final double? arcSweepOverride;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (trackColor != null) {
      ...
      if (hasGap && value != null && value! > _kEpsilon) {
        ...
        final double endGap = value! < _kEpsilon ? startGap : startGap * 2;
        final double startSweep = (-math.pi / 2.0) + startGap;
        final double endSweep = math.max(
          0.0,
          _kTwoPi - clampDouble(value!, 0.0, 1.0) * _kTwoPi - endGap,
        );
        ...
        canvas.drawArc(arcBaseOffset & arcActualSize, 0, _kSweep, false, backgroundPaint);
      }
    }
  ...
  bool shouldRepaint(_CircularProgressIndicatorPainter oldPainter) {
    return oldPainter.trackColor != trackColor ||
        ...
        oldPainter.arcStartOverride != arcStartOverride ||
        oldPainter.arcSweepOverride != arcSweepOverride;
  }
}
...
class _CircularProgressIndicatorState extends State<CircularProgressIndicator>
  ...
  late AnimationController _transitionController;
  _IndicatorMode _mode = _IndicatorMode.indeterminate;
  Animation<double>? _arcStartAnimation;
  Animation<double>? _arcSweepAnimation;
  ...
  void initState() {
    ...
    _mode = widget.value == null ? _IndicatorMode.indeterminate : _IndicatorMode.determinate;
    if (_mode == _IndicatorMode.indeterminate) {
      _controller.repeat();
    }
  }
  ...
  double _currentIndeterminateArcStart() {
    return _kCircularStartAngle +
        _strokeTailTween.evaluate(_controller) * 3 / 2 * math.pi +
        _rotationTween.evaluate(_controller) * math.pi * 2.0 +
        _offsetTween.evaluate(_controller) * 0.5 * math.pi;
  }

  double _currentIndeterminateArcSweep() {
    final double head = _strokeHeadTween.evaluate(_controller);
    final double tail = _strokeTailTween.evaluate(_controller);
    return math.max(head * 3 / 2 * math.pi - tail * 3 / 2 * math.pi, _kEpsilon);
  }

  // Returns the arc start/sweep currently being drawn so we can retarget a
  // new tween without visually snapping when updates arrive mid-transition.
  (double arcStart, double arcSweep) _currentArcStateForDeterminate(double fallbackValue) {
    if ((_mode == _IndicatorMode.transitioningToDeterminate ||
            _mode == _IndicatorMode.transitioningToIndeterminate) &&
        _transitionController.isAnimating &&
        _arcStartAnimation != null &&
        _arcSweepAnimation != null) {
      return (_arcStartAnimation!.value, _arcSweepAnimation!.value);
    }

    if (_mode == _IndicatorMode.indeterminate) {
      return (_currentIndeterminateArcStart(), _currentIndeterminateArcSweep());
    }

    return (_kCircularStartAngle, _kSweep * _clampValue(fallbackValue));
  }

  void _resetTransitionAnimations(Tween<double> startTween, Tween<double> sweepTween) {
    _transitionController.stop();
    _arcStartAnimation = startTween.chain(CurveTween(curve: _effectiveTransitionCurve)).animate(_transitionController)
      ..addListener(() => setState(() {}));
    _arcSweepAnimation = sweepTween.chain(CurveTween(curve: _effectiveTransitionCurve)).animate(_transitionController);
  }

  void _startTransitionToDeterminate(double targetValue) {
    _controller.stop();
    final (double beginArcStart, double beginArcSweep) =
        _currentArcStateForDeterminate(targetValue);
    final double endArcStart = _kCircularStartAngle;
    final double endArcSweep = _kSweep * _clampValue(targetValue);

    _resetTransitionAnimations(
      Tween<double>(begin: beginArcStart, end: endArcStart),
      Tween<double>(begin: beginArcSweep, end: endArcSweep),
    );
  ...
  void _startTransitionToIndeterminate(double fromValue) {
    _controller.stop();
    final (double beginArcStart, double beginArcSweep) =
        _currentArcStateForDeterminate(fromValue);
    final double endArcStart = beginArcStart;
    const double endArcSweep = 0.0;
  ...
  void _startTransitionBetweenDeterminate(double fromValue, double toValue) {
    ...
    final (double beginArcStart, double beginArcSweep) =
        _currentArcStateForDeterminate(fromValue);
    final double endArcStart = _kCircularStartAngle;
    final double endArcSweep = _kSweep * _clampValue(toValue);
  ...
  Widget _buildMaterialIndicator(
    ...
    {double? arcStartOverride, double? arcSweepOverride}
  ) {
    final ProgressIndicatorThemeData indicatorTheme = ProgressIndicatorTheme.of(context);
    final bool year2023 = widget.year2023 ?? indicatorTheme.year2023 ?? true;
    final ProgressIndicatorThemeData defaults = switch (Theme.of(context).useMaterial3) {
    ...
    Widget result = ConstrainedBox(
      ...
      child: CustomPaint(
        painter: _CircularProgressIndicatorPainter(
          ...
          trackGap: trackGap,
          year2023: year2023,
          arcStartOverride: arcStartOverride,
          arcSweepOverride: arcSweepOverride,
        ),
      ),
    );
  ...
  Widget build(BuildContext context) {
    switch (widget._indicatorType) {
      case _ActivityIndicatorType.material:
        return _buildForMaterial(context);
  ...
  Widget _buildForMaterial(BuildContext context) {
    if (_mode == _IndicatorMode.transitioningToDeterminate ||
      ...
      return _buildMaterialIndicator(
        ...
        0.0,
        0.0,
        arcStartOverride: _arcStartAnimation?.value,
        arcSweepOverride: _arcSweepAnimation?.value,
      );
    }
